...
data class SchemaLoaderImpl(
  ...
  override fun withPreloadedDocument(preloadedSchema: kotlinx.serialization.json.JsonObject): SchemaReader {
  ...
  override operator fun plus(preloadedSchema: kotlinx.serialization.json.JsonObject): SchemaReader {
    JsonUtils.extractIdFromObject(preloadedSchema)?.also { id ->
      documentClient.registerLoadedDocument(id, preloadedSchema)
    }
    return this
  ...
  override operator fun plusAssign(preloadedSchema: kotlinx.serialization.json.JsonObject) {
    JsonUtils.extractIdFromObject(preloadedSchema)?.also { id ->
      documentClient.registerLoadedDocument(id, preloadedSchema)
    }
  }

 ...
 */
class PhoneFormatValidator : FormatValidator {

  override fun validate(subject: String): String? {
    try {
      return if (FormatChecks.isValidPhone(subject)) {
        null
      } else {
        "[%s] is not a valid phone number".format(subject)
      }
    } catch (e: Exception) {
      return "[%s] is not a valid phone number".format(subject)
    }
  }
